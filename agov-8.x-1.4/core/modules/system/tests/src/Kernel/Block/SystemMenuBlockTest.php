...
class SystemMenuBlockTest extends KernelTestBase {
  ...
  public function testConfigExpanded() {
    ...
    $expectations['level_1_and_beyond'] = $expectations['all'];
    ...
    foreach ($blocks as $id => $block) {
      $block_build = $block->build();
      $items = isset($block_build['#items']) ? $block_build['#items'] : [];
      $this->assertSame($expectations[$id], $this->convertBuiltMenuToIdTree($items), 'Menu block "' . $id . '" with an active trail renders the expected tree.');
    }
  }
