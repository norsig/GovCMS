...
<form enctype='multipart/form-data' id="event-form" method="post" action="<?php echo esc_url(add_query_arg(array('success'=>null))); ?>">
	<div class="wrap">
		...
		<?php if(get_option('dbem_events_anonymous_submissions') && !is_user_logged_in()): ?>
			...
			<div class="inside event-form-submitter">
				<p>
					...
					<input class="form-control" type="text" name="event_owner_name" id="event-owner-name" value="<?php echo esc_attr($EM_Event->event_owner_name); ?>" />
				</p>
				<p>
					<label><?php esc_html_e('Email', 'events-manager'); ?></label>
					<input class="form-control" type="text" name="event_owner_email" id="event-owner-email" value="<?php echo esc_attr($EM_Event->event_owner_email); ?>" />
				</p>
		...
		<div class="inside event-form-name">
			<input class="form-control" type="text" name="event_name" id="event-name" value="<?php echo esc_attr($EM_Event->event_name,ENT_QUOTES); ?>" /><?php echo $required; ?>
			<br />
		...
		<div class="inside event-form-details form-group">
			<div class="event-editor">
				...
				<?php else: ?>
					<textarea class="form-control" name="content" rows="10" style="width:100%"><?php echo $EM_Event->post_content ?></textarea>
					<br />
		...
		<div class="inside event-form-bookings form-group">				
	...
	<p class="submit">
	    ...
	    <input type='submit' class='btn btn-primary button-primary' value='<?php echo esc_attr(sprintf( __('Submit %s','events-manager'), __('Event','events-manager') )); ?>' />
	    <?php else: ?>
	    <input type='submit' class='btn btn-primary button-primary' value='<?php echo esc_attr(sprintf( __('Update %s','events-manager'), __('Event','events-manager') )); ?>' />
	    <?php endif; ?>
	</p>
